         ...
         <a wire:click="logout" class="list-group-item active" style="cursor: pointer">LOGOUT<i class="fas fa-sign-out-alt"></i></a>
             ...
             <div class="bg-cover rounded-xl" style="background-image: url(https://images.unsplash.com/photo-1560250056-07ba64664864?q=80&w=2902&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D); background-position: center;" class="">
                <div class="bg-gradient-to-r from-black p-10 rounded-xl">
                    <h1 class="font-bold text-4xl text-white">🤖 Halaman Kategori </h1>
                </div>
             </div>
         <div>
            <section class="mt-4">
                            ...
                            <table class="w-full text-md text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b dark:border-gray-200">
                                            ...
                                            <td class="px-4 text-left text-black py-3"><span class="text-md  leading-8 font-semibold p-1 rounded-2xl">{{ucwords(strtolower($category->name))}}</span></td>
                                            {{-- <td class="px-4 text-center text-black py-3">
                                                <img class="w-[70px]" src="{{asset('storage/assets/clients_images/' . $client->image)}}" alt="">
                                            </td> --}}
                                            <td scope="row" class="px-1 py-3 flex flex-col items-start justify-center text-gray-900 font-semibold"><a class="" href="{{route('admin.categories.edit.{id}', $category->id)}}">Edit ✏️</a> <button wire:click="destroy({{$category->id}})">Hapus 🗑️</button></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
